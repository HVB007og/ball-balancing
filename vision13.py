...
# Default PID constants .5,1e-08,38
kp = 0
ki = 0
kd = 0
        ...
        if contours:
            ...
            if contours:
                ...
                if 0.8 < aspect_ratio < 1.2:  # Ball should be approximately circular
                    ...
                    nx = ((kp * x_error) + (ki * sum_error[0]) + (kd * diff_error[0])) * scalingfact
                    ny = ((kp * y_error) + (ki * sum_error[1]) + (kd * diff_error[1])) * scalingfact
                    nmag = math.sqrt((nx ** 2) + (ny ** 2) + 1)

                    print(f"NX:  {nx:.5f} NY:  {ny:.5f} NMAG: {nmag:.5f}")
                    nx /= nmag
                    ny /= nmag
                    nz = 1 / nmag
                    ...
                    message = f'{offfn(theta_a)},{offfn(theta_b)},{offfn(theta_c)}'
                    print(f"pubvalues: {offfn(theta_a)},{offfn(theta_b)},{offfn(theta_c)} ")
