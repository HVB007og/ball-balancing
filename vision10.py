...
def Publisher():
    ...
    while not rospy.is_shutdown():
        ...
        # Filter small noise by applying morphological operations
        mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, np.ones((5, 5), np.uint8))  # Remove small noise
        mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, np.ones((10, 10), np.uint8))  # Close gaps in objects
        ...
        if contours:
            ...
            if area > 300:  # Only process if the contour is large enough
